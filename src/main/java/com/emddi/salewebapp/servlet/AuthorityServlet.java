...
import com.emddi.salewebapp.util.Cmd;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			...
			if(type.equals("1"))//query
			{
				...
				//devices
				Map<String, Map<String, Object>> devs = Db.deviceMap;
...
//									"DATA DELETE userauthorize Pin=1001\r\nPin=20150186\r\n";
									...
									Cmd.addDeviceCommand(sn, sb.toString());
									sb.setLength(0);
						...
						if(!add.trim().equals(""))
						{
							...
							for(String sn : snSet)
							{
								...
								if(empInfo.length()>0)
								{
									Cmd.addDeviceCommand(sn, empInfo.toString());
									Cmd.addDeviceCommand(sn, empAuth.toString());
									Cmd.addDeviceCommand(sn, "DATA UPDATE timezone TimezoneId="+groupTime+timezoneCmd(Db.timeMap.get(groupTime)));
									empInfo.setLength(0);
									empAuth.setLength(0);
								}
							}
